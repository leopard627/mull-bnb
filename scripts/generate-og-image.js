...
async function generateOGImage() {
  ...
  // Background gradient - BNB Chain brand colors (golden/orange)
  const gradient = ctx.createLinearGradient(0, 0, width, height);
  gradient.addColorStop(0, "#F0B90B");
  gradient.addColorStop(0.5, "#F3BA2F");
  ...
  // Add dark accent circles
  ctx.fillStyle = "rgba(30, 32, 38, 0.1)";
  ctx.beginPath();
  ctx.arc(width * 0.9, height * 0.9, 150, 0, Math.PI * 2);
  ctx.fill();
  ...
  // Subtitle - dark text for golden background
  ctx.fillStyle = "rgba(30, 32, 38, 0.9)";
  ...
  tags.forEach((tag) => {
    // Tag background - dark for contrast
    ctx.fillStyle = "rgba(30, 32, 38, 0.2)";
    ctx.beginPath();
